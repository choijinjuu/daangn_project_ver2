...
<body>
	...
	<div id="fleaDe_wrap">
		...
		<div class="flea_all" id="fleaDe_title_area">
			...
			<div id="flea_id_area">
				<div id="flea_id"><a><strong>${b.title }</strong></a></div>
				<div id="flea_address">${b.subCategory }  ∙  조회수 ${b.count }</div>
			</div>
			<div id="flea_btn_area">
				<c:choose>
					<c:when test="${not empty loginMember and loginMember.memNo == b.memNo}">
						<!-- 로그인 되어있으며 게시글 작성자일때 -->
						<button type="button" onclick="location.href='updateForm.bo?category=2&boardNo=${b.boardNo}'">수정</button>
		...
		<div class="flea_all" id="fleaDe_text_area">
			...
			<div id="flea_map_area">
				<div id="map_text_area">
					...
					<div id="map_text_area_2"><span>${b.address }</span></div>
				</div>
				<div id="map_area"></div>
			</div>
			<div id="flea_time_area">
				<div id="time_img_area"><img src="resources/css_img/time.png"></div>
				<div id="time_text"><span>${b.openTime } - ${b.closeTime }</span></div>
			</div>
		</div>
		<hr style="border: 0.1px solid rgb(224, 224, 224); width: 680px; margin: auto;">
		<%-- <%for (Reply r : rlist) {%>
	...
	<script>
		...
		function boardDelete(){
			...
			if(result){
				alert("삭제되었습니다.");
				location.href = "delete.bo?category=2&boardNo=${b.boardNo}&atPath=${b.attachment.atPath }";
			}else{
				alert("삭제가 취소되었습니다.");
			}
		}
	    ...
	    // 주소로 좌표를 검색
	    geocoder.addressSearch('${b.address}', function(result, status) {
