...
<body>
							...
							<div class="box6">
								<div id="div_email" class="box_inn selected">
									...
									<div class="box_inn_sub">
										<p class="dsc">회원가입시 입력한 이름과 이메일 주소가 같아야, 인증번호를 받을 수 있습니다.</p>	
										<dl>
											...
											<dd class="ct" style="margin-top: -10px;">
												<span class="input_box2">
													...
													<p id="searchId_Text" style="font-size: 13px; color: red; margin-left: 140px;"></p>
												</span>								
											</dd>	
										</dl>
									</div>		
	      ...
	      <div class="modal-body" id="searchId_Modal">
			<div class="box_inn_sub">
				...
				<dl style="position: relative; top: 10px;">
					...
					<dd><input type="text" id="searchIdGo" name="searchIdGo" class="input_txt" readonly><br></dd>
				</dl>
			</div>
	      </div>
	      <div class="modal-footer">
	      	<button type="button" onclick="location.href='searchPwdForm.me'" style="background-color: white; border-radius: 5px; width: 120px;">비밀번호 찾기</button>
	...
	<script>
		...
		function searchId(){
			...
			console.log("들어옴");
			console.log($("#emailNm").val());
			console.log($("#email").val());
			$.ajax({
				url : "searchMe.me",
				data : {memName : $("#emailNm").val(),
						email : $("#email").val()
						},
			...
			$.ajax({
				...
				success : function(result){
					...
					if(result.ranNum != ""){
						alert("인증 번호가 발송되었습니다.");
						$("#emailAuthNo").prop("disabled",false);
						$("#searchIdGo").val(result.ranNum);
						$("#emailAuthNo").focus();
					}else if(result.ranNum == ""){
						$("#searchId_Text").show();
						$("#searchId_Text").text("일치하는 회원 정보가 없습니다.");
					}
				},error : function(){
		...
		//{"ranNum":"5578","memId":"user01"}
		
		
		/* 찾은 아이디 모달에 전달 */
		
		$(function(){
			//일치하는 회원 없을떄 뜬 경고 메세지 지워주기
			$("#emailNm").click(function(){
				$("#searchId_Text").hide();
			});
